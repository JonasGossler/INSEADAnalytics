...
1. You need 6 principal factors (52.29 % variance explained)

2. 5 year T-Notes (0.93), 10 year T-Notes (US) (0.93) and Treasury Bonds (0.86) 

3. Bonds, Currencies, European Stock Exchanges, North American Stock Exchanges, Euro-related futures, Oil & Gas, Metals, Commodities, Precious Metals, Asian Stock Exchanges

4. 
crisis_data = futures_data[as.Date(rownames(futures_data)) > "2007-10-01" & as.Date(rownames(futures_data)) < "2009-03-01", ])  or 
bull_data = futures_data[as.Date(rownames(futures_data)) > "2002-11-01" & as.Date(rownames(futures_data)) < "2007-09-30", ])
Then we must only replace all "futures_data" in the subsequent code by crisis_data or bull_data and the results will adjust auromatically when the HTML is knitted

5. The analysis shows higher correlation of financial assets in crisis years compared to bull years (as comprehensive economic trends impact the global economy and industry- / region-specific differences diminish). The scree plot is steeper, the Eigenvalues are higher (one component explains relatively more variance).  
...
1. The code calculates multiple moving averages in intervals of 20 days (10 day moving average up to 250 day). Those moving averages (or their signs) are summed up for each future. These sums are then scaled across all futures in the portfolio to get an absolute value of 1. This means that each day, 1$ is invested in either short or long positions of those futures. Some are sold, some purchased dependent on the weight of their scaled moving averages.

2. Decisions to go short or long might change dependent on duration and amount of averages used. Therefore, also returns will change, making the choice of proper indicators one of the most crucial success factors in trend following. In this case, if the interval would be increased from 20 to 10 (using double the amount of moving averages), the return would slightly increase.
