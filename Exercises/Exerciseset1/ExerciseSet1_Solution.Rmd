...
**Your Answers here:** https://github.com/buchen/portfolio a tool for portfolio analysis
<br>
<br>
...
1. mytickers = c("SPY", "AAPL") defines a vector called "mytickers" which contains all downloaded tickers. If further companies' data is required, one can easily add further elements to that vector.
2.  The formulas used are the same as for SPY (2 instead of 1 is put in the bracket as the second value of the vector is used instead of the first)
    Cumulative returns: round(100*sum(StockReturns[,2]),1) = 597.3   
    Average daily returns: round(100*mean(StockReturns[,2]),3) 0.148   
    Standard deviation of the daily returns: round(100*sd(StockReturns[,2]),3) = 2.389
3. We have to alter the above mentioned vector and add "YHOO": mytickers = c("SPY", "AAPL", "YHOO"). Sourcing the dataSet1.R again after adding "YHOO" gives us the following results for Yahoo:
    Cumulative returns: 290.7  
    Average daily returns: 0.072   
    Standard deviation of the daily returns: 3.018
<br>
<br>
<br>
<br>
...
1. nrow(transposedData), ncol(transposedData)
2. portfolio2 = apply(transposedData,2,mean), pnl_plot(portfolio2)
...
**Your Answers here:**
1. startDate = "2001-01-01"
In my dataSet1.R the date was already set to 2001. If not, one would have to change it accordingly.

2. Repeat the Yahoo-exercise for any S&P 500 company of interest
<br>
<br>
<br>
<br>
...
**Your Answers here:**
1. I get 60 as a result - what should not be because the datasets should be equal (no inequal numbers to add up)
2. Error in FUN(left, right) : non-numeric argument to binary operator. Reason for that error and also the "wrong" answer in 1. is that myData contains the dates as column, which is not the case in StockReturns
<br>
<br>
<br>
<br>
